...
func (p Protos) RegisterProvider(rtype string) error {
	req, err := http.NewRequest("POST", p.createURL("provider/"+rtype), bytes.NewBuffer([]byte{}))
	if err != nil {
		return err
	}
...
func (p Protos) DeregisterProvider(rtype string) error {
	req, err := http.NewRequest("DELETE", p.createURL("provider/"+rtype), bytes.NewBuffer([]byte{}))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
...
func (p Protos) GetResources() (map[string]*resource.Resource, error) {
	...
	resources := Resources{}
	resourcesReq, err := http.NewRequest("GET", p.createURL("resource/provider"), nil)
	if err != nil {
